...
ANTHROPIC_API_URL = "https://generativelanguage.googleapis.com/v1beta/models/ANTHROPIC-1.5-flash:generateContent"
...
def call_ANTHROPIC(prompt, system):
    print("DEBUG: Checking for ANTHROPIC_API_KEY in environment...")
    print("DEBUG: Available env keys:", [k for k in os.environ.keys() if not k.startswith("npm") and not k.startswith("RUNNER")])

    api_key = os.environ.get("ANTHROPIC_API_KEY")

    if not api_key:
        print("DEBUG: ANTHROPIC_API_KEY not found! Trying alternate names...")
        for key in os.environ.keys():
            if "ANTHROPIC" in key.upper() or "API" in key.upper():
                print("DEBUG: Found possible key:", key)
        raise ValueError("ANTHROPIC_API_KEY not set in environment")

    print("DEBUG: ANTHROPIC_API_KEY found, length=" + str(len(api_key)))
    ...
    resp = requests.post(
        ANTHROPIC_API_URL + "?key=" + api_key,
        json=payload,
        headers={"Content-Type": "application/json"}
    )

    print("DEBUG: ANTHROPIC API response status:", resp.status_code)

    if resp.status_code != 200:
        print("DEBUG: ANTHROPIC error response:", resp.text)
...
def generate_post(topic):
    ...
    result = call_ANTHROPIC(prompt, POST_SYSTEM)
...
def generate_diagram(topic, diagram_type):
    ...
    result = call_ANTHROPIC(prompt, DIAGRAM_SYSTEM)
