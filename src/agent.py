...
# ─── TOPIC SANITY GUARD ───────────────────────────────────────────────────────
# Catches nonsensical cross-domain "X vs Y" topics generated by the trending
# picker (e.g. "Kubernetes vs CSS"). Both sides of a comparison must belong to
# the same technical domain or at least one must be unclassified.

_TOPIC_DOMAINS = {
    "container_orchestration": ["kubernetes", "k8s", "openshift", "rancher", "nomad", "ecs", "helm"],
    "styling":                 ["css", "sass", "less", "tailwind", "bootstrap", "styled-components", "postcss"],
    "databases":               ["postgres", "postgresql", "mysql", "mariadb", "mongodb", "redis", "cassandra",
                                "dynamodb", "sqlite", "clickhouse", "bigquery", "snowflake", "neo4j"],
    "languages":               ["python", "javascript", "typescript", "java", "golang", "go ", "rust",
                                "ruby", "kotlin", "swift", "c++", "c#", "scala", "elixir", "php"],
    "cloud_platforms":         ["aws", "azure", "gcp", "google cloud", "heroku", "vercel", "netlify",
                                "cloudflare", "fly.io"],
    "ai_ml_frameworks":        ["pytorch", "tensorflow", "jax", "keras", "hugging face", "langchain",
                                "llamaindex", "openai", "anthropic", "llm", "rag", "llama", "mistral"],
    "messaging_streaming":     ["kafka", "rabbitmq", "sqs", "pubsub", "nats", "pulsar", "kinesis"],
    "frontend_frameworks":     ["react", "vue", "angular", "svelte", "nextjs", "next.js", "nuxt",
                                "remix", "astro", "htmx"],
    "containerisation":        ["docker", "podman", "containerd", "buildkit", "kaniko"],
...
def _topic_domain(term: str) -> str:
    t = term.lower()
...
def _is_sane_comparison_topic(topic_name: str) -> bool:
    """
    Return False when a "X vs Y" topic crosses unrelated technical domains.
    Single-domain and non-comparison topics always pass.
    """
    if " vs " not in topic_name.lower():
        return True
    sides = re.split(r"\s+vs\.?\s+", topic_name, flags=re.I)
    if len(sides) < 2:
        return True
    left_d  = _topic_domain(sides[0])
    right_d = _topic_domain(sides[1])
    if left_d == "unknown" or right_d == "unknown":
        return True   # can't classify → don't block
    if left_d != right_d:
        log.warning(
            f"Nonsensical comparison topic blocked: '{topic_name}' "
            f"({sides[0].strip()!r}={left_d} vs {sides[1].strip()!r}={right_d})"
        )
        return False
    return True
...
- NEGATIVE CONSTRAINT: Never use prose sub-headers like "Why It Matters:", "The Core Idea:", "What Works:", "The Fix:", "The Takeaway:", "In Production:", "So What:" — these read like a template outline, not a post. Integrate the insight directly into the paragraph that follows.
...
# Pattern 1: "Label: description" lines
    STRUCTURE_ECHO_PATTERN = re.compile(
        ...
        r"Serve|Govern|Deploy|Observe|Scale|Build|Test|Operate|"
        # ── Extended: prose-header labels the LLM writes as sub-headings ──
        r"Why It Matters|Why This Matters|The Core Idea|What Works|"
        r"What Doesn'?t Work|The Real Problem|The Hard Part|The Fix|"
        r"The Takeaway|The Insight|The Lesson|The Pattern|The Mistake|"
        r"What I Learned|What Changed|What to Do|The Solution|"
        r"In Practice|In Production|The Bottom Line|So What)\s*:.*$",
...
def _post_quality_issues(topic, post_text, structure=None, diagram_type=""):
    ...
    naked_label_re = re.compile(
        ...
        r"How It Works|Key Takeaway|Summary|Introduction|"
        r"Why It Matters|Why This Matters|The Core Idea|What Works|"
        r"What Doesn'?t Work|The Real Problem|The Hard Part|The Fix|"
        r"The Takeaway|The Insight|The Lesson|The Pattern|The Mistake|"
        r"What I Learned|What Changed|What to Do|The Solution|"
...
def _has_structural_integrity_issues(text):
    ...
    naked_label_pattern = re.compile(
        r"^(Best Practices|Common Mistakes|The Problem|Core Concept|"
        r"How It Works|Key Takeaway|Summary|Introduction|Hook|"
        r"Why It Matters|Why This Matters|The Core Idea|What Works|"
        r"What Doesn'?t Work|The Real Problem|The Hard Part|The Fix|"
        r"The Takeaway|The Insight|The Lesson|The Pattern|The Mistake|"
        r"What I Learned|What Changed|What to Do|The Solution|"
...
"""
        ...
        try:
            ...
            if trends:
                ...
                if trend_topic:
                    # Sanity-check: reject nonsensical cross-domain comparisons
                    # (e.g. "Kubernetes vs CSS" — real prod case that broke posts)
                    if not _is_sane_comparison_topic(trend_topic.get("name", "")):
                        log.warning(
                            f"Trending topic '{trend_topic['name']}' rejected by "
                            f"comparison sanity guard — falling back to topic mode"
                        )
                        mode = "topic"
                    else:
                        topic = trend_topic
