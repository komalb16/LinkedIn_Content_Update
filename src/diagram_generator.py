...
def flow_box(fx, fy, fw, fh, ico, title, sub, color):
    """Icon + label flow cell — dark fill, colored border."""
    s  = f'<rect x="{fx}" y="{fy}" width="{fw}" height="{fh}" rx="8" fill="{color}18" stroke="{color}" stroke-width="1.5"/>'
    iw = fw - 10
    if ico:
        s += f'<text x="{fx+fw//2}" y="{fy+20}" text-anchor="middle" font-size="15" font-family="Arial,sans-serif">{ico}</text>'
        s += f'<text x="{fx+fw//2}" y="{fy+35}" text-anchor="middle" fill="{color}" font-size="{min(10,max(7,iw//7))}" font-weight="700" font-family="Arial,sans-serif">{x(clamp(title,iw//5))}</text>'
        if sub and fh > 50:
            s += f'<text x="{fx+fw//2}" y="{fy+48}" text-anchor="middle" fill="{TEXT_MID}" font-size="{min(8,max(6,iw//9))}" font-family="Arial,sans-serif">{x(clamp(sub,iw//4))}</text>'
    else:
        mid = fy + fh // 2
        s += f'<text x="{fx+fw//2}" y="{mid+(0 if not sub else -5)}" text-anchor="middle" fill="{color}" font-size="{min(11,max(7,iw//7))}" font-weight="700" font-family="Arial,sans-serif">{x(clamp(title,iw//5))}</text>'
        if sub:
            s += f'<text x="{fx+fw//2}" y="{mid+11}" text-anchor="middle" fill="{TEXT_MID}" font-size="{min(9,max(6,iw//9))}" font-family="Arial,sans-serif">{x(clamp(sub,iw//4))}</text>'
    return s
...
def tool_strip(ty, tools, color):
    ...
    for i, (ico, name) in enumerate(tools):
        cx = 15 + i*tw + tw//2
        s += f'<text x="{cx}" y="{ty+10}" text-anchor="middle" font-size="11" font-family="Arial,sans-serif">{ico}</text>'
        s += f'<text x="{cx}" y="{ty+22}" text-anchor="middle" fill="{color}" font-size="{min(9,max(6,tw//9))}" font-weight="700" font-family="Arial,sans-serif">{x(clamp(name,tw//5))}</text>'
        if i < n-1:
            lx = 15+(i+1)*tw
...
def cheatrow(ry, rh, label, color, items):
    ...
    for i, item in enumerate(items):
        ...
        if ico: s += f'<text x="{cx2+cw//2}" y="{ry+14}" text-anchor="middle" font-size="12" font-family="Arial,sans-serif">{ico}</text>'
        s += f'<text x="{cx2+cw//2}" y="{ry+rh//2+(0 if not ico else 7)}" text-anchor="middle" fill="{color}" font-size="{min(9,max(6,cw//8))}" font-weight="700" font-family="Arial,sans-serif">{x(clamp(title2,cw//5))}</text>'
...
def wrap(content, title, subtitle, color, date_str):
  ...
  {content}
  <!-- footer -->
  <rect x="0" y="552" width="900" height="28" fill="{BG2}"/>
  <line x1="0" y1="552" x2="900" y2="552" stroke="{BORDER2}" stroke-width="1"/>
  <text x="20" y="570" fill="{TEXT_LITE}" font-size="9" font-family="Arial,sans-serif">{x(date_str)} · Generated by AI</text>
  <rect x="648" y="558" width="238" height="16" rx="8" fill="{color}20" stroke="{color}60" stroke-width="1"/>
  <text x="767" y="570" text-anchor="middle" fill="{color}" font-size="9" font-weight="700" font-family="Arial,sans-serif" letter-spacing="0.5">✦ AI · © Komal Batra</text>
</svg>'''
...
def make_system_design(C):
    s = tool_strip(70, [("🌐","React/Next"),("🔀","Nginx/ALB"),("🔑","OAuth2"),("🚀","K8s"),("🗄️","PostgreSQL"),("📡","Redis"),("📊","Grafana")], C[0])
    s += section(15, 98, 870, 46, "CLIENT LAYER", C[0])
    for i,(ico,t) in enumerate([("🌐","Browser"),("📱","Mobile"),("💻","Desktop"),("🤝","3rd Party"),("📟","IoT")]):
        cx = 15+i*174+87
        s += f'<text x="{cx}" y="{114}" text-anchor="middle" font-size="14" font-family="Arial,sans-serif">{ico}</text>'
        s += f'<text x="{cx}" y="{132}" text-anchor="middle" fill="{C[0]}" font-size="9" font-weight="700" font-family="Arial,sans-serif">{t}</text>'
...
def make_lakehouse(C):
    s = tool_strip(70, [("🔥","Spark"),("❄️","Delta Lake"),("🧊","Iceberg"),("🌊","Flink"),("🔵","dbt"),("📊","Trino"),("🗂️","Hive")], C[0])
...
def make_kubernetes(C):
    s = tool_strip(70, [("☸️","kubectl"),("🔧","Helm"),("📊","Prometheus"),("🔍","Jaeger"),("🔐","RBAC"),("🌐","Ingress")], C[0])
...
def make_llm(C):
    s = tool_strip(70, [("🤖","Transformer"),("🔗","LangChain"),("📊","vLLM"),("🔍","RAG"),("🛡️","Guardrails"),("📡","API")], C[0])
...
def make_cicd(C):
    s = tool_strip(70, [("📝","Git"),("🏗️","GitHub Actions"),("🐳","Docker"),("☸️","K8s"),("🔍","SonarQube"),("📊","Grafana")], C[0])
    phases = [("CODE","💻","git push","pre-commit"),("BUILD","🏗️","compile","unit tests"),
    ...
    for i,(env,ico,phase,tools) in enumerate(phases):
        ...
        s += f'<text x="{bx+(pw-3)//2}" y="{114}" text-anchor="middle" fill="{col}" font-size="9" font-weight="700" font-family="Arial,sans-serif">{x(env)}</text>'
        s += f'<text x="{bx+(pw-3)//2}" y="{162}" text-anchor="middle" font-size="24" font-family="Arial,sans-serif">{ico}</text>'
        s += f'<text x="{bx+(pw-3)//2}" y="{184}" text-anchor="middle" fill="{col}" font-size="9" font-weight="700" font-family="Arial,sans-serif">{x(clamp(phase,10))}</text>'
        s += f'<text x="{bx+(pw-3)//2}" y="{198}" text-anchor="middle" fill="{TEXT_MID}" font-size="8" font-family="Arial,sans-serif">{x(clamp(tools,12))}</text>'
...
def make_zero_trust(C):
    s = tool_strip(70, [("🔐","mTLS"),("🛡️","OPA"),("👁️","SIEM"),("🔑","Vault"),("🌐","ZTNA"),("📋","PAM")], C[0])
...
def make_aws(C):
    s = tool_strip(70, [("☁️","VPC"),("🔐","IAM"),("⚖️","ALB"),("⚡","Lambda"),("🗄️","RDS"),("📊","CloudWatch")], C[0])
...
def make_mlops(C):
    s = tool_strip(70, [("🔬","MLflow"),("🏭","Kubeflow"),("📊","Evidently"),("🔵","dbt"),("☁️","S3"),("🚀","Seldon")], C[0])
...
def make_rag(C):
    s = tool_strip(70, [("🔍","Embed"),("🗄️","VectorDB"),("✂️","Chunking"),("🔗","LangChain"),("📊","Reranker"),("🛡️","Guardrails")], C[0])
...
def make_devsecops(C):
    s = tool_strip(70, [("🔒","SAST/DAST"),("🐳","Trivy"),("☸️","Falco"),("📋","Semgrep"),("🛡️","OPA"),("📊","Splunk")], C[0])
    phases=[("IDE","💻","Pre-commit","git-secrets"),("SCM","📝","Review","SAST scan"),
            ("Build","🏗️","Compile","SCA/SBOM"),("Test","🧪","Quality","DAST/ZAP"),
            ("Artifact","📦","Registry","Trivy+sign"),("Stage","🚀","Deploy","IaC scan"),("Prod","🛡️","Runtime","Falco/eBPF")]
    pw = 870 // len(phases)
    for i,(env,ico,phase,tools) in enumerate(phases):
        col = C[i%len(C)]; bx = 15+i*pw
        ...
        s += f'<text x="{bx+(pw-3)//2}" y="{113}" text-anchor="middle" fill="{col}" font-size="9" font-weight="700" font-family="Arial,sans-serif">{x(env)}</text>'
        s += f'<text x="{bx+(pw-3)//2}" y="{158}" text-anchor="middle" font-size="24" font-family="Arial,sans-serif">{ico}</text>'
        s += f'<text x="{bx+(pw-3)//2}" y="{176}" text-anchor="middle" fill="{col}" font-size="9" font-weight="700" font-family="Arial,sans-serif">{x(clamp(phase,10))}</text>'
        s += f'<text x="{bx+(pw-3)//2}" y="{190}" text-anchor="middle" fill="{TEXT_MID}" font-size="8" font-family="Arial,sans-serif">{x(clamp(tools,12))}</text>'
...
def make_docker(C):
    ...
    rows=[("FROM / BASE",C[0],[("🐧","Alpine","3.19"),("🐧","Ubuntu","22.04 LTS"),("🟢","Node","20-alpine"),("🐍","Python","3.12-slim"),("☕","JDK","21-eclipse")]),
          ("BUILD",C[1],[("📋","COPY","src→workdir"),("⚡","RUN","apt/pip install"),("🔧","ARG","build-time var"),("💾","CACHE","layer reuse"),("🏗️","Multi-stage","slim final")]),
          ("CONFIGURE",C[2],[("🌍","ENV","runtime vars"),("📂","WORKDIR","app dir"),("👤","USER","non-root"),("📁","VOLUME","mount points"),("🔌","EXPOSE","port decl")]),
          ("RUNTIME",C[3],[("🚀","CMD","default exec"),("⚙️","ENTRYPOINT","fixed cmd"),("🏥","HEALTHCHECK","liveness"),("📡","LABEL","metadata"),("🛑","STOPSIGNAL","graceful")])]
    y = 106
...
def make_git_workflow(C):
    s = tool_strip(70, [("🌿","Git"),("🔀","Merge"),("🏷️","Tag"),("🔍","Review"),("🤖","Actions"),("📋","Conventional")], C[0])
    branches=[("main","🛡️","protected","prod deploy",C[0]),("develop","🔧","integration","staging deploy",C[1]),
              ("feature/*","✨","short-lived","PR target",C[2]),("release/*","🏷️","RC branch","freeze",C[3]),("hotfix/*","🚨","critical fix","direct merge",C[4])]
    bw = 870 // len(branches)
    for i,(name,ico,role,deploy,col) in enumerate(branches):
        ...
        s += f'<text x="{bx+(bw-4)//2}" y="{132}" text-anchor="middle" font-size="26" font-family="Arial,sans-serif">{ico}</text>'
...
def make_api_design(C):
    ...
    for i,(name,ico,proto,use,col) in enumerate(paradigms):
        ...
        s += f'<text x="{bx+(pw-3)//2}" y="{132}" text-anchor="middle" font-size="26" font-family="Arial,sans-serif">{ico}</text>'
...
def make_solid(C):
    s = tool_strip(70, [("📐","SOLID"),("🏗️","Clean Arch"),("🔧","DI"),("🧪","TDD"),("📦","DDD"),("🔌","Ports/Adapters")], C[0])
...
def make_generic(topic_name, C):
    s = tool_strip(70, [("🏗️","Architecture"),("⚙️","Services"),("🔌","APIs"),("🗄️","Data"),("🔐","Security"),("📊","Observability")], C[0])
