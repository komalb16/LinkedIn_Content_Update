...
def make_flow_svg(topic_name, topic_id, color, now):
    tid = topic_id.lower()
    if "cicd" in tid:
        steps = [("Code Push", "git push"), ("Lint & Test", "pytest/eslint"), ("Build", "docker build"), ("Scan", "trivy/snyk"), ("Deploy Staging", "helm upgrade"), ("Integration Test", "selenium"), ("Deploy Prod", "blue/green")]
    elif "mlops" in tid:
    ...
    if row2:
        ...
        arrows += f'<path d="M {x_end} 230 Q {x_end+60} 310 650 320" stroke="{color}" stroke-width="2" fill="none" stroke-dasharray="5,3" marker-end="url(#arr)" opacity="0.6"/>'
...
def make_cheatsheet_svg(topic_name, topic_id, color, now):
    ...
    if "docker" in tid:
        ...
        items = [
            ...
            ("DRY", "Dont Repeat Yourself", "Every piece of knowledge has one place"),
        ]
    else:
        items = [
...
def make_fallback_svg(topic_name, topic_id, diagram_type):
    ...
    safe_name = topic_name.replace("&", "and").replace("<", "").replace(">", "")
  ...
  <rect x="678" y="519" width="204" height="26" rx="13" fill="url(#siggrad)" opacity="0.15"/>
  <rect x="679" y="520" width="202" height="24" rx="12" fill="none" stroke="url(#siggrad)" stroke-width="0.8" opacity="0.6"/>
  <text x="693" y="536" fill="#A29BFE" font-size="10" font-family="Arial,sans-serif" filter="url(#glow)">&#10022; AI</text>
...
class DiagramGenerator:
    ...
    def save_svg(self, svg_content, topic_id, topic_name="", diagram_type="Architecture Diagram"):
        ...
        log.info("Generating diagram for: " + topic_id)
